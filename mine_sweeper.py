...
pygame.init()
SURFACE = pygame.display.set_mode([WIDTH*SIZE+200, HEIGHT*SIZE])
...
def main():
    ...
    while True:
        ...
        # 그리기
        SURFACE.fill((0, 0, 0))
        score_image = smallfont.render("score = {}".format(OPEN_COUNT),
                                     True, (0, 0, 225))
        SURFACE.blit(score_image, (WIDTH*SIZE+10,10))
        Timer_image = smallfont.render("Time = {:.1f}".format(timer),
                                     True, (0, 0, 225))
        SURFACE.blit(Timer_image, (WIDTH*SIZE+10,40))
        for ypos in range(HEIGHT):
